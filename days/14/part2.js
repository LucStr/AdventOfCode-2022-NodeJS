...
const MAX_DEPTH = Math.max(...map.map(e => e.y)) + 2;
...
main: do {
    ...
    for(let direction of DIRECTIONS){
        ...
        if(!existing){
            existing = {x: next.x, y: next.y, type: next.y < MAX_DEPTH ? '.' : '#', previous: tile};
            map.push(existing);
        }
...
require('fs').writeFileSync(__dirname + '/map.txt', printMap());
