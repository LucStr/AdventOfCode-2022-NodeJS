...
function printMap(){
    let minX = map.reduce((acc, elf) => Math.min(acc, elf.x), Infinity);
    let maxX = map.reduce((acc, elf) => Math.max(acc, elf.x), -Infinity);
    let minY = map.reduce((acc, elf) => Math.min(acc, elf.y), Infinity);
    let maxY = map.reduce((acc, elf) => Math.max(acc, elf.y), -Infinity);
...
const result = (maxX - minX + 1) * (maxY - minY + 1) - map.length;
console.log(result)
